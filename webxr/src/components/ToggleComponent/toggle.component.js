...
AFRAME.registerComponent("my-toggle", {
  schema: {
  ...
  init: function () {
    ...
    this.backboxMaterial = new THREE.MeshLambertMaterial({
      color: this.data.active ? this.activeColor : this.inactiveColor,
    });
    this.knobMaterial = new THREE.MeshLambertMaterial({
      color: this.data.knobColor,
    });
    this.knobGrabbedMaterial = new THREE.MeshLambertMaterial({
      color: 0x7cc7ed,
      emissive: 0x46b1ff,
    });
      ...
      el.addEventListener("mouseenter", () => {
        ...
        setTimeout(() => {
          if (data.stillHovering) {
            ...
            if (data.active) {
              el.emit("active");
              data.knob.position.set(data.knobTranslation, 0, 0);
              data.setColor(data.knobTranslation);
            } else {
              el.emit("inactive");
              data.knob.position.set(-data.knobTranslation, 0, 0);
              data.setColor(-data.knobTranslation);
    ...
    const titleText = new MeshText2D(this.data.title, {
    ...
    this.chassis.add(titleText);

    this.el.setObject3D("mesh", this.chassis);

    this.controllers = Array.prototype.slice.call(
      document.querySelectorAll("a-entity[hand-controls]")
    );
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        this.knob.position.set(
          this.data.active ? this.knobTranslation : -this.knobTranslation,
          0,
          0
        );
  ...
  tick: function (time, timeDelta) {
    if (this.grabbed) {
      ...
      var handWorld = new THREE.Vector3().setFromMatrixPosition(
        hand.matrixWorld
      );
